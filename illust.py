...
class Illust(ChatGPT):
    ...
    def __init__(
        self, app, deepl_api_key, automatic1111_domain, chatgpt_api_key, queue
    ):
    ...
    def register_message_handler(self):
        ...
        self.app.message(re.compile(self.SIGNATURE_I2I_PROMPT, re.S))(
            self.message_i2i_prompt
        )
        self.app.message(re.compile(self.SIGNATURE_I2I2_PROMPT, re.S))(
            self.message_i2i_prompt
        )
        ...
        self.app.message(re.compile(self.SIGNATURE_QUEUE_CHECK, re.S))(
            self.message_queue_check
        )
        ...
        while True:
            try:
                ...
                if item is None:
                    ...
                    url = self.automatic1111(
                        item["word"], item["negative"], item["seed"]
                    )
                else:
                    self.worker_count = 1
                    self.worker_queue = item["prompt"]
                ...
                self.worker_queue = ""
                item["say"](
                    blocks=self.response(url, item["word"], item["prompt"]),
                    thread_ts=item["ts"],
    ...
    def response(self, url, prompt, jp_prompt=None):
        ...
        return [
            {
                "type": "image",
                "title": {"type": "plain_text", "text": jp_prompt},
                "block_id": "image4",
                "image_url": url,
                "alt_text": prompt,
            }
